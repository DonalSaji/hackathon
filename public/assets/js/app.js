...
class ThemeCustomizer {
    ...
    initSwitchListener() {
        ...
        hoverBtn.forEach(function (element) {
            element.addEventListener("click", function () {
                const currentSize = window.config.sidenav.size;
                const currentAttr = document.documentElement.getAttribute("data-sidenav-size") || currentSize;
        
                if (currentAttr === "sm-hover-active") {
                    window.changeLeftbarSize("sm-hover", false);
                    sessionStorage.setItem("sidebarSize", "sm-hover");
                } else {
                    window.changeLeftbarSize("sm-hover-active", false);
                    sessionStorage.setItem("sidebarSize", "sm-hover-active");
                }
            });
        })
    }
    ...
    _adjustLayout() {
        ...
        if (window.innerWidth <= 767.98) {
            ...
            console.log(self.config.sidenav.size);

...
<body>
    ...
    <div class="wrapper">
        ...
        <!-- PWA Install Notification (Bottom-Center) -->
        {{-- <div id="pwa-install-banner" class="bg-body-secondary rounded-3"
            style="display:none; position:fixed; bottom:20px; left:50%;
            transform: translateX(-50%) translateY(40px);
            opacity:0; transition:all 0.3s ease;
            width:250px; padding:12px 14px; text-align:center;
            z-index:10000;">

            <!-- Floating X Button Outside Box -->
            <button id="pwa-close-btn"
                style="position:absolute; top:-8px; right:-8px;
               width:22px; height:22px;
               background-color:#f01818;color:#fff;
            ...
            <div class="fw-semibold mb-2" style="font-size:13px;">
                Start using the Arya app today!
            </div>

            <div class="d-flex justify-content-center">
                <button id="pwa-install-btn" class="btn btn-outline-success btn-sm py-1 px-3" style="font-size:12px;">
                    Install
                </button>
            </div>
        </div> --}}

        @include('backend.body.scripts')
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                if (!sessionStorage.getItem('device_width_sent')) {
                    fetch("{{ url('/store-device-width') }}", {
                        method: "POST",
                        headers: {
                        ...
                        sessionStorage.setItem('device_width_sent', true);
                    });
                }
            });
        </script>
